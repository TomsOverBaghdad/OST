...
public class HttpUtils {
    private static final String TAG = "httputils";//LogTag.TRANSACTION;
    ...
    public static byte[] httpConnection(Context context, long token,
        ...
        try {
            ...
            Log.e(TAG, "req header "+ req.toString());
            ...
            Log.e(TAG, "target " + target.toString() + " req "+ req.getParams().toString());
    ...
    private static AndroidHttpClient createHttpClient(Context context) {
        String userAgent = MmsConfig.getUserAgent();/*((TelephonyManager)context
                .getSystemService(Context.TELEPHONY_SERVICE))
                .getMmsUserAgent();*/
        ...
        int soTimeout = 60*1000;//default to 1 min //MmsConfig.getHttpSocketTimeout();

        if (true){//(Log.isLoggable(LogTag.TRANSACTION, Log.DEBUG)) {
